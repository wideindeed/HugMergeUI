"""Round Seven left one confound unresolved: both parent/child pairs tested
there (Coder-1.5B->Coder-1.5B-Instruct, Math-1.5B->Math-1.5B-Instruct) are
also domain-divergent fine-tunes, a category already known (Round Four/Five)
to blow up perplexity for reasons unrelated to parent/child structure. So
that result couldn't distinguish "parent/child chains are fine" from
"domain-divergence dominates and happens to swamp the parent/child effect."

This is the missing data point: a parent/child chain in a NON-domain-
divergent category. Qwen2.5-1.5B-Instruct -> huihui-ai/Qwen2.5-1.5B-
Instruct-abliterated is exactly that - the abliterated model is fine-tuned
FROM the instruct model (parent/child), but "abliteration" (refusal-
direction ablation) doesn't shift the domain at all: it's still a general
chat model, not a coder/math specialist. Same base (Qwen2.5-1.5B), same
fp32 methodology, same subprocess-isolation design as phase_hierarchy.py,
scored against the true family base like every other pair on record.
"""
...
PAIRS = [
    dict(
        family_name="qwen_instruct_1.5b",
        base_repo="Qwen/Qwen2.5-1.5B",
        name_a="instruct",
        repo_a="Qwen/Qwen2.5-1.5B-Instruct",
        name_b="abliterated",
        repo_b="huihui-ai/Qwen2.5-1.5B-Instruct-abliterated",
    ),
]

RESULTS_PATH = Path(__file__).resolve().parent / "phase_hierarchy2_results.json"
CONFIGS_DIR = Path(__file__).resolve().parent / "configs" / "hierarchy2"
...
def run_pair_isolated(spec: dict, timeout_s: int = 1800, retries: int = 1) -> dict | None:
    ...
    for attempt in range(1, retries + 2):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            worker_script = Path(__file__).resolve().parent / "phase_hierarchy2_worker.py"
