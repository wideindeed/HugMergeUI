"""Round Twelve follow-up (ad hoc, single pair): every domain-divergent
pair tested so far crosses one domain fine-tune against the plain instruct
model (instruct+coder, instruct+math) or is a parent/child chain within one
domain (Coder-1.5B -> Coder-1.5B-Instruct, Math-1.5B -> Math-1.5B-Instruct).
Nobody has crossed two independently-trained, domain-divergent siblings
against EACH OTHER. This is that pair: Qwen2.5-Coder-1.5B-Instruct x
Qwen2.5-Math-1.5B-Instruct, both instruction-tuned off Qwen2.5-1.5B, pulling
in unrelated directions. Same subprocess-isolation design and fp32
methodology as phase_hierarchy2.py.

Originally ran from a D:\ drive copy (VALIDATION.txt Round Thirteen)
with family_name "qwen_1.5b_cross_domain"; renamed to "qwen1.5b" here
to pool with the rest of that family's data in
phase_scale_1_5b_results.json and phase_pooled_meta.py.
"""
...
PAIRS = [
    dict(
        family_name="qwen1.5b",
        base_repo="Qwen/Qwen2.5-1.5B",
        name_a="coder_instruct",
        repo_a="Qwen/Qwen2.5-Coder-1.5B-Instruct",
        name_b="math_instruct",
        repo_b="Qwen/Qwen2.5-Math-1.5B-Instruct",
    ),
]

RESULTS_PATH = Path(__file__).resolve().parent / "phase_cross_domain_results.json"
CONFIGS_DIR = Path(__file__).resolve().parent / "configs" / "cross_domain"
...
def run_pair_isolated(spec: dict, timeout_s: int = 1800, retries: int = 1) -> dict | None:
    ...
    for attempt in range(1, retries + 2):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            worker_script = Path(__file__).resolve().parent / "phase_cross_domain_worker.py"
